...
def enable(name, tool_type=ToolType.INCLUSIVE, ignore_error_on_error=True):
	try:
		...
		if valid:
			Tools[name] = tool
			_shortName(name)
			FishLog.debug("Loaded toolchain", name, log_level=2)
		else:
			if ignore_error_on_error:
				FishLog.warning("Tool does not meet system dependencies. Continuing without...", tool=name)
		...
		FishLog.error("Error loading module", module=name, error=e)
		if not ignore_error_on_error:
			raise e
	except ImportError as e:
		FishLog.error("Error importing module", module=name)
		if not ignore_error_on_error:
			raise e
	FishLog.warning("Continuing without toolchain", name)
